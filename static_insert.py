...
def static_data_insert(name):
    ...
    filename = name + '.xlsx'
    table_name = 'generic.table_columns'
    ...
    check_data_query = "SELECT COUNT(1) FROM " + table_name + " WHERE TABLE_NAME = 'generic." + name + "'"
    cur.execute(check_data_query)
    data_count = cur.fetchone()
    print('There are ' + str(data_count[0]) + ' rows in table ' + table_name)

    if data_count[0] != 0:
        delete_query = 'DELETE FROM ' + table_name + " WHERE TABLE_NAME = 'generic." + name + "'"
    ...
    insert_query = 'INSERT INTO ' + table_name + '(TABLE_NAME, COLUMN_NAME, COLUMN_NUMBER) VALUES (?,?,?)'
    print(insert_query)
