...
/****** Object:  StoredProcedure [new].[SP_DIM_RESPONDENT_ANSWER]    Script Date: 2020-04-09 12:49:52 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER                   PROCEDURE [new].[SP_DIM_RESPONDENT_ANSWER]
AS
BEGIN
...
FROM
generic.question_answers_list WHERE TABLE_NAME LIKE 'generic.new_%') BASE

SELECT @DATA_COUNT = COUNT(1) FROM ##Question

WHILE @CNT_QUES <= @DATA_COUNT
BEGIN

SELECT @TABLE_NAME = TABLE_NAME, @QUESTION = QUESTION, @QUESTION_HEADING = QUESTION_HEADING FROM ##Question WHERE ROW_ID = @CNT_QUES
...
FROM new.DimAnswer DA 
JOIN new.DimQuestion DQ ON DA.QUESTION_ID = DQ.QUESTION_ID
...
FROM
new.DimAnswer DA
JOIN new.DimQuestion DQ ON DQ.QUESTION_ID = DA.QUESTION_ID
...
SET @UNION_QUERY = 'SELECT * INTO ##QuestionRespondent FROM ( ' + SUBSTRING(@UNION_QUERY, 12, LEN(@UNION_QUERY)) + ' ) BASE '

DROP TABLE IF EXISTS ##QuestionRespondent

EXECUTE (@UNION_QUERY)

SELECT @TABLE_COUNT = COUNT(1) FROM new.DimRespondentAnswer

IF(@TABLE_COUNT > 0)
BEGIN
DELETE new.DimRespondentAnswer
END

INSERT INTO new.DimRespondentAnswer
SELECT 
FORMAT(GETDATE(), 'yyyyMMdd') AS CYCL_TIME_ID,
RESPONDENT_ID,
QUESTION_ANSWER_ID 
FROM ##QuestionRespondent
