...
def load_data_new(survey, time):
    i_log_file = './logs/' + 'Data_Load_Logging' + '_' + cycl_time_id + '.txt'
    FORMAT = '%(message)s'
    ...
    generic_tablename = '[' + SCHEMA_NAME['generic'] + '].[' + time + '_' + survey + '_survey_date]'
    extension = filename.split(".")[1]

    if extension == 'xlsx':
        data = pd.read_excel('./sourcefiles/' + survey + '/' + filename, header=[0,1])
    if extension == 'csv':
